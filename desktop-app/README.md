...
The Linux runner is generated by the pinned Flutter tool rather than hand-maintained in this repository. CI and release workflows run the same `flutter create --platforms=linux` step before analysis/build so plugin CMake wiring stays in sync with the selected Flutter version.
