...
func (p *SubsonicPlugin) SetupRoutes(mux *http.ServeMux) {
	...
	routes := map[string]http.HandlerFunc{
		...
		"/rest/createPlaylist":    p.auth(p.handleCreatePlaylist),
		"/rest/updatePlaylist":    p.auth(p.handleUpdatePlaylist),
		"/rest/deletePlaylist":    p.auth(p.handleDeletePlaylist),
		"/rest/search2":                   p.auth(p.handleSearch3),
		"/rest/search3":                   p.auth(p.handleSearch3),
		"/rest/getRandomSongs":            p.auth(p.handleGetRandomSongs),
